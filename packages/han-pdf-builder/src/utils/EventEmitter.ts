...
export class EventEmitter {
   ...
   */
  on<T = unknown>(type: string, listener: EventListener<T>, options: EventListenerOptions = {}): string {
   ...
   */
  once<T = unknown>(type: string, listener: EventListener<T>, options: EventListenerOptions = {}): string {
   ...
   * Emit an event
   */
  async emit<T = unknown>(event: T): Promise<void> {
    const type = (event as { type?: string }).type || 'default';
    const listeners = this.listeners.get(type);
   ...
   */
  emitSync<T = unknown>(event: T): void {
    const type = (event as { type?: string }).type || 'default';
    const listeners = this.listeners.get(type);
